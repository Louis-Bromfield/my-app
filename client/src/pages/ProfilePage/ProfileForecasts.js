...
function ProfileForecasts(props) {
    ...
    return (
        <div className="profile-forecasts">
            <h2 className="profile-header">{props.searched === true ? `${props.playerUsername}'s Races` : `Races`}</h2>
            {(brierReverse[0].problemName === null || props.userObj.brierScores.length === 0) &&
                <h3>No scores here; scores achieved for forecasting a problem found on the "Races" page will be shown here.</h3>
            }
            {brierReverse[0].problemName !== null &&
                <div>
                    <h3>Number of Races Attempted: {brierReverse.length}</h3>
                    {brierReverse.map((item, index) => {
                        return (
                            <div key={item.problemName+index} index={item.problemName} className="profile-forecasts-individual-result">
                                <div className="problem-header"><h4 style={{ color: "#404d72" }}>Race:&nbsp;</h4><h4>{item.problemName}</h4></div>
